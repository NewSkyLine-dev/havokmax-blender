...
def parse_bytes(data: bytes, override_name: Optional[str] = None) -> HavokPack:
    ...
    packfile_bytes = _unwrap_bytes(data)
    xml_root = _parse_xml_root(packfile_bytes)
    skeleton = _parse_skeleton(xml_root, override_name)
    animations = _parse_animations(xml_root, skeleton)
    meshes = _parse_meshes(xml_root, override_name)
    return HavokPack(skeleton=skeleton, animations=animations, meshes=meshes)


def _parse_xml_root(packfile_bytes: bytes) -> ET.Element:
    """Locate and parse the embedded Havok XML from a packfile.

    Havok packfiles often start with a short binary header followed by an XML
    payload. Instead of assuming the XML starts at offset zero, scan for the
    first XML marker and parse from there. This mirrors the approach used by
    HavokMax, which walks the packfile container to reach an
    ``hkaAnimationContainer`` before loading skeletons/animations.
    """

    try:
        return ET.fromstring(packfile_bytes)
    except ET.ParseError:
        trimmed = _slice_embedded_havok(packfile_bytes)
        if trimmed is None:
            raise ValueError("Unsupported or corrupt Havok payload")
        try:
